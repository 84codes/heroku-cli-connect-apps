    ...
    async run() {
        try {
            ...
            for (let app in appData) {
                ...
                let connectome = apps_1.default.connectMap[apps_1.default.appNameToKey(appName)];
                if (connectome == null) {
                    continue;
                }
                for (let candidateApp in appData) {
                    ...
                    let candidateConnectome = connectome[apps_1.default.appNameToKey(candidateAppName)];
        ...
        if (connections.length > 0) {
            ...
            for (let connection of connections) {
                if (currTo === connection.to) {
                    let tab = new Array(app.length + connection.to.length + 10).join(' ');
